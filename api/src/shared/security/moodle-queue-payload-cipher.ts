...
const PURPOSE = 'moodle-queue-payload-v1';
let cachedKey: Buffer | undefined;

/**
 * Encrypts/decrypts data that travels through BullMQ job payloads (persisted
 * in Redis in transit). Uses a subkey derived from
 * MOODLE_CACHE_ENCRYPTION_KEY distinct from the one used for cache-at-rest,
 * so the two purposes never share key material directly.
 */
