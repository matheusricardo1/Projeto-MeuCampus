...
export class ListRestaurantsUseCase {
    ...
    async execute(credentials: RuDigitalCredentials): Promise<Restaurant[] | PendingScrapeJob> {
        try {
            return await this.cache.listRestaurants(credentials.cpf);
        } catch (error) {
            if (!(error instanceof RuDigitalResourceNotFoundException)) {
                throw error;
            }

            await this.scrapingJobService.enqueue('restaurant-list', { credentials }, {
                dedupeKey: scrapingJobDedupeKey(credentials, 'ru-digital-restaurant-list')
            });
            return pendingScrapeJob('restaurant-list');
        }
    }
}
