...
import { encryptCachePayload } from '@/infrastructure/crypto/moodle-cache-cipher';
import { decryptQueuePayload, encryptQueuePayload } from '@/infrastructure/crypto/moodle-queue-payload-cipher';
...
describe('moodle-cache-cipher (worker encrypt side)', () => {
    beforeAll(() => {
        process.env.MOODLE_CACHE_ENCRYPTION_KEY = TEST_SECRET;
    });

    it("encrypts a payload decryptable by anyone deriving the same moodle-cache-v1 subkey — this is exactly what the API's independent decryptCachePayload does on the other side", () => {
        const payload = { courses: [{ id: 1, shortName: 'X1' }] };
...
describe('key derivation fails fast when misconfigured', () => {
    const ORIGINAL_ENV = process.env.MOODLE_CACHE_ENCRYPTION_KEY;

    afterAll(() => {
        if (ORIGINAL_ENV === undefined) {
            delete process.env.MOODLE_CACHE_ENCRYPTION_KEY;
        } else {
            process.env.MOODLE_CACHE_ENCRYPTION_KEY = ORIGINAL_ENV;
        }
    });

    it('throws instead of silently encrypting with an undefined secret when MOODLE_CACHE_ENCRYPTION_KEY is missing', async () => {
        delete process.env.MOODLE_CACHE_ENCRYPTION_KEY;
        vi.resetModules();
