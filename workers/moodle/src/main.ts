...
import { MoodleSyncWorker } from '@/infrastructure/queue/bullmq-moodle-sync.worker';
...
process.on('uncaughtException', (error) => {
    appLogger.critical(`Moodle sync worker crashed: uncaught exception - ${error.message}`, {
...
process.on('unhandledRejection', (reason) => {
    ...
    appLogger.critical(`Moodle sync worker crashed: unhandled rejection - ${message}`, {
...
async function bootstrap() {
    const worker = new MoodleSyncWorker();

    const shutdown = async (signal: string) => {
        appLogger.info(`Received ${signal}. Closing Moodle sync worker...`);
        await worker.close();
        process.exit(0);
    };
...
bootstrap().catch((error: unknown) => {
    ...
    appLogger.critical(`Moodle sync worker failed to start: ${message}`);
    process.exitCode = 1;
});
