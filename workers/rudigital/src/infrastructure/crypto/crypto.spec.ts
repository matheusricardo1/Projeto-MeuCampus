...
import { encryptCachePayload } from '@/infrastructure/crypto/ru-digital-cache-cipher';
import { decryptQueuePayload, encryptQueuePayload } from '@/infrastructure/crypto/ru-digital-queue-payload-cipher';
...
describe('ru-digital-cache-cipher (worker encrypt side)', () => {
    beforeAll(() => {
        process.env.RUDIGITAL_CACHE_ENCRYPTION_KEY = TEST_SECRET;
    });

    it('encrypts a payload decryptable by anyone deriving the same ru-digital-cache-v1 subkey — this is exactly what the API\'s independent decryptCachePayload does on the other side', () => {
        const payload = { breakfast: { mealPrice: 0.75, currentBalance: 0, availableForPurchase: 26 } };
        const encrypted = encryptCachePayload(payload);

        const key = deriveKey(TEST_SECRET, 'ru-digital-cache-v1');
        expect(decryptJson(key, encrypted)).toEqual(payload);
    });
});

describe('ru-digital-queue-payload-cipher', () => {
    beforeAll(() => {
        process.env.RUDIGITAL_CACHE_ENCRYPTION_KEY = TEST_SECRET;
    });

    it('round-trips a payload through encrypt/decrypt', () => {
        const payload = { session: { token: 'abc', restaurantId: 'MAO' } };
    ...
    it('is cryptographically isolated from the cache-cipher purpose: a cache-derived key cannot decrypt a queue-payload ciphertext', () => {
        ...
        const cacheKey = deriveKey(TEST_SECRET, 'ru-digital-cache-v1');
        expect(() => decryptJson(cacheKey, encrypted)).toThrow();
    });
});

describe('key derivation fails fast when misconfigured', () => {
    const ORIGINAL_ENV = process.env.RUDIGITAL_CACHE_ENCRYPTION_KEY;

    afterAll(() => {
        if (ORIGINAL_ENV === undefined) {
            delete process.env.RUDIGITAL_CACHE_ENCRYPTION_KEY;
        } else {
            process.env.RUDIGITAL_CACHE_ENCRYPTION_KEY = ORIGINAL_ENV;
        }
    });

    it('throws instead of silently encrypting with an undefined secret when RUDIGITAL_CACHE_ENCRYPTION_KEY is missing', async () => {
        delete process.env.RUDIGITAL_CACHE_ENCRYPTION_KEY;
        vi.resetModules();
        const freshModule = await import('./ru-digital-cache-cipher.js');

        expect(() => freshModule.encryptCachePayload({ any: 'value' }))
            .toThrow('CRITICAL: RUDIGITAL_CACHE_ENCRYPTION_KEY must be defined.');
    });
});
