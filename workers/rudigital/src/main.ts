...
import { RuDigitalScrapingWorker } from '@/infrastructure/queue/bullmq-ru-digital-scraping.worker';
...
process.on('uncaughtException', (error) => {
    appLogger.critical(`RU Digital scraping worker crashed: uncaught exception - ${error.message}`, {
...
process.on('unhandledRejection', (reason) => {
    ...
    appLogger.critical(`RU Digital scraping worker crashed: unhandled rejection - ${message}`, {
...
async function bootstrap() {
    const worker = new RuDigitalScrapingWorker();

    const shutdown = async (signal: string) => {
        appLogger.info(`Received ${signal}. Closing RU Digital scraping worker...`);
        await worker.close();
        process.exit(0);
    };
...
bootstrap().catch((error: unknown) => {
    ...
    appLogger.critical(`RU Digital scraping worker failed to start: ${message}`);
    process.exitCode = 1;
});
