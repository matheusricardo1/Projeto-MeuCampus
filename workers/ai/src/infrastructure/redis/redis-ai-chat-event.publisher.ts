...
import type { AiChatChunkEvent, AiChatEventPublisher, AiChatFailedEvent, AiChatReadyEvent, AiChatToolEvent } from '@/application/ports/ai-chat-event-publisher';
...
export class RedisAiChatEventPublisher implements AiChatEventPublisher {
    ...
    async publishTool(event: AiChatToolEvent): Promise<void> {
        await this.publisher.publish(AI_CHAT_RESULT_CHANNEL, JSON.stringify(event));
    }
